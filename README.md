...
_Se genera la matriz de betas son de 8 columnas por que son 2 modelos y 4 tipo de poblaciones_
set.seed(150)
repeticiones=10000
...
_En esta parte lo que hace es que recorre cada tipo de poblacion, de acuerdo a las x sub i que existan._

for (j in 1:length(n)) {
  x1=rnorm(n[j],20,1)
  x2=(0.8*x1)+rnorm(n[j],0,1)
  
  _En esta parte es un for por lo cual aqui hace las 10.000 repeticiones de acuerdo a los modelos que se dan_
